...
if ( !class_exists("class_DL_bruker"))
{
    ...
    class class_DL_bruker extends class_DL
    {
        ...
        public static function get_trans(class_dbpdo $DB)
        {
		  ...
		  public function get_user($iBrukerId)
        {
			  try
            {
					//class_log::logg("DEBUG", __FILE__.' '.__LINE__);
					//$this->DB->beginTransaction();
                $aBruker = self::get_user_trans($this->DB, $iBrukerId);
					//class_log::logg("DEBUG", __FILE__.' '.__LINE__);
					//$this->DB->commit();
            }
		  ...
		  public static function get_user_trans(class_dbpdo $DB, $iBrukerId)
        {
			  $sSql = "SELECT BrukerId,Fornavn,Mellomnavn,Etternavn,Epost,Mobil,Passord,BrukerType,Brukerbetingelser,OpprettetDato,Aktiv FROM Bruker WHERE BrukerId=:BrukerId";
			  $aRows = $DB->query_select($sSql, [ ':BrukerId' => $iBrukerId ] );
           return ($aRows);
        }
		  ...
		  public function update_user(data_bruker $oBruker)
		  {
			  try
            {
					//class_log::logg("DEBUG", __FILE__.' '.__LINE__);
					//$this->DB->beginTransaction();
                $bBruker = self::update_user_trans($this->DB, $oBruker);
					//class_log::logg("DEBUG", __FILE__.' '.__LINE__);
					//$this->DB->commit();
            }
            catch (PDOException $ex)
            ...
            return (new data_ret(__FILE__, __LINE__, class_error::ERROR_OK, true, $bBruker ));
		  }
		  
		  public static function update_user_trans(class_dbpdo $DB, data_bruker $oBruker) 
		  {
			   $sSql = "UPDATE Bruker SET Fornavn=:Fornavn,Mellomnavn=:Mellomnavn,Etternavn=:Etternavn,Epost=:Epost,Mobil=:Mobil WHERE BrukerId=:BrukerId";
				$DB->query_execute($sSql, [
					':Fornavn'=>$oBruker->sFornavn,
					':Mellomnavn'=>$oBruker->sMellomnavn,
					':Etternavn'=>$oBruker->sEtternavn,
					':Epost'=>$oBruker->sEpost,
				   ':Mobil'=>$oBruker->sMobil,
					':BrukerId'=>$oBruker->iBrukerId
            ]);
			  return true;
		  }
		  
		public function update_password($iBrukerId, $sNewPassword)
		 {
			try
			 {
				  $bBruker = self::update_password_trans($this->DB, $iBrukerId, $sNewPassword);
			 }
			 catch (PDOException $ex)
			 {
				  $this->oLog->dbexception_log($ex);
		...
		public static function update_password_trans(class_dbpdo $DB, $iBrukerId, $sNewPasswordr) 
		{
			$sSql = "UPDATE Bruker SET Passord=:Passord WHERE BrukerId=:BrukerId";
			$DB->query_execute($sSql, [ ':Passord'=> $sNewPasswordr, ':BrukerId' => $iBrukerId ]);
			return true;
		}
	}
}
